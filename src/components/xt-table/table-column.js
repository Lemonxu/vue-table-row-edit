...
export default {
  ...
  render (h) {
    const rules = {};
  // <table-row data={[]} rowIndex={1} row={{}} prop={this.column.prop} v-model={row[column.prop]} rules={rules} rule={rules[column.prop]} column={column}></table-row>
    return <td>111</td>;
  },
  methods: {
  //   获取table传回来的值 主要为:data="data" :rowIndex="rowIndex" :row="row" :rules="rules"
    getTableData() {
      Bus.$on("data", (data) => {
        this.data = data;
      });
